                ...
                {/* search filter from Hero Component */}
                {
                    isSearched && (searchFilter.title !=""|| searchFilter.location !="") && (
                        ...
                        <div className="mb-4 text-grey-600">
                            {searchFilter.title && (
                                <span className="inline-flex items-center gap-2.5 bg-blue-50 border border-blue-200 rounded-full px-4 py-1.5 rounded">
                                    {searchFilter.title}
                                    <img  onClick={ e =>setSearchFilter(prev =>({...prev,title:""}))} className="cursor-pointer" src={assets.cross_icon} alt="" />
                                </span>
                            )}
                             {searchFilter.location && (
                                <span className="ml-2 inline-flex items-center gap-2.5 bg-red-50 border border-red-200 rounded-full px-4 py-1.5 rounded">
                                    {searchFilter.location}
            ...
            <section className="w-full lg:w-3/4 text-gray-800 max-lg:px-4">
                ...
                {filteredJobs.length > 0 && (
                    <div className="flex items-center justify-center space-x-2 mt-10">
                        <a href="#job-list">
                            <img onClick={e => setCurrentPage(Math.max(currentPage-1,1))} src={assets.left_arrow_icon} alt="" />
                        </a>
                        {Array.from({length:Math.ceil(filteredJobs.length/6)}).map((_,index)=>(
                         ...
                         <a href="#job-list">
                            <img onClick={e => setCurrentPage(Math.min(currentPage+1,Math.ceil(filteredJobs.length/6)))} src={assets.right_arrow_icon} alt="" />
                        </a>
