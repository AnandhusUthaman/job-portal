...
const ManageJobs = () => {
  ...
  // FIX: Initialize jobs as an empty array
...
//functiuon to change job visibility 
const changeJobVisibility = async (id) => {
  ...
  return jobs ? jobs.length === 0 ? (<div className="flex items-center justify-center h-[70vh]"> 
    <p className=" flex justify-center items-center text-xl sm:text-2xl h-full text-center text-gray-500"> No Jobs Available or Posted Yet</p>
  </div>) : (
            ...
            {jobs.map((job,index)=>(
              <tr key={index} className="text-gray-700"> 
                ...
                <td className="py-2 px-4 border-b border-gray-200 text-center">
                  <input type="checkbox"  className="scale-125 ml-4"  checked={job.visible} onChange={()=>changeJobVisibility(job._id)} />
                </td>
              </tr>
            ))}
