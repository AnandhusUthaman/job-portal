...
const ViewApplication = () => {
  ...
  const fetchCompanyJobApplications = async () => {
    ...
    try {
      console.log('Fetching applicants with token:', companyToken)
  ...
  return applicants ? applicants.length === 0 ? (<div className="flex items-center justify-center h-[70vh]"> 
    <p className=" flex justify-center items-center text-xl sm:text-2xl h-full text-center text-gray-500"> No Applicants Available</p>
  </div>) : (
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className="text-gray-700">
                ...
                <td className="py-2 px-4 border-b ">
                  {applicant.userId && applicant.userId.resume ? (
