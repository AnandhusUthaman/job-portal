        ...
        <div className='flex gap-2 mb-6 mt-3'>
          {
            ...
            : userData && userData.resume ? (
              ...
              <div className='flex gap-2'>
                <button onClick={()=>setIsEdit(true)} className='bg-blue-100 text-blue-600 px-4 py-2 rounded-lg'>
                  Upload Resume
                </button>
              </div>
            )
            ...
            {userApplications && userApplications.length > 0 ? userApplications.map((job, index)=> (
              <tr key={index}>
                ...
                <td className='py-2 px-4 border-b'> 
                    <span className={`${job.status === 'Accepted' ? 'bg-green-100 text-green-600' : job.status === 'Rejected' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'} px-4 py-1.5 rounded`}> 
                      {job.status} 
                    </span>
                  </td>
              </tr>
